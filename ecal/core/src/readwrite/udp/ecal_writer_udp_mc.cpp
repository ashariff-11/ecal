...
namespace eCAL
{
  ...
  bool CDataWriterUdpMC::Write(const void* const buf_, const SWriterAttr& attr_)
  {
    ...
    if (attr_.loopback)
    {
      ...
      {
        sent = m_sample_sender_loopback->Send(m_ecal_sample.topic().tname(), m_ecal_sample);
      }
    }
    else
    {
      ...
      {
        sent = m_sample_sender_no_loopback->Send(m_ecal_sample.topic().tname(), m_ecal_sample);
      }
    }
